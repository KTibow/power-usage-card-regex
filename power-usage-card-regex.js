...
class PowerUsageCardRE extends HTMLElement {
  ...
  set hass(hass) {
    ...
    const ctx = canvas.getContext("2d");
    var entlist = Object.values(hass.states).filter(
      (e) =>
        e.entity_id.match(config.filter) &&
        typeof e.state !== "undefined" &&
        e.state.match(/^[0-9\.]+$/)
    );
    const hassEntities = entlist.map((y) => hass.states[y]);
    let entityNames = hassEntities.map((x) => x.attributes.friendly_name);
    let entityData = hassEntities.map((x) => x.state);
    card.header = config.title ?? "Power usage graph";

    if (config.total_power_usage) {
      const totalEntity = hass.states[config.total_power_usage];
      const total =
        totalEntity.attributes.unit_of_measurement == "kW"
          ? totalEntity.state * 1000
          : totalEntity.state;
      const measured = hassEntities
    ...
    const emptyIndexes = entityData.reduce(
